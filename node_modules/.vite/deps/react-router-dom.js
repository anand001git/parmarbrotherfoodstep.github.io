...
function createRouter(init) {
	...
	async function handleAction(request, location, submission, matches, opts) {
		if (opts === void 0) opts = {};
		interruptActiveLoads();
		updateState({ navigation: getSubmittingNavigation(location, submission) });
		let result;
		...
		if (!actionMatch.route.action && !actionMatch.route.lazy) result = {
			...
			result = await callLoaderOrAction("action", request, actionMatch, matches, manifest, mapRouteProperties$1, basename);
	...
	async function handleLoaders(request, location, matches, overrideNavigation, submission, fetcherSubmission, replace, pendingActionData, pendingError) {
		...
		if (matchesToLoad.length === 0 && revalidatingFetchers.length === 0) {
			...
			}, pendingActionData ? { actionData: pendingActionData } : {}, updatedFetchers$1 ? { fetchers: new Map(state.fetchers) } : {}));
			return { shortCircuited: true };
		}
		if (!isUninterruptedRevalidation) {
			...
			updateState(_extends$2({ navigation: loadingNavigation }, actionData ? Object.keys(actionData).length === 0 ? { actionData: null } : { actionData } : {}, revalidatingFetchers.length > 0 ? { fetchers: new Map(state.fetchers) } : {}));
		}
		...
		if (redirect$1) {
			await startRedirectNavigation(state, redirect$1, { replace });
			return { shortCircuited: true };
		}
	...
	async function handleFetcherAction(key, routeId, path, match, requestMatches, submission) {
		...
		let fetcher = getSubmittingFetcher(submission, state.fetchers.get(key));
		state.fetchers.set(key, fetcher);
		updateState({ fetchers: new Map(state.fetchers) });
		let abortController = new AbortController();
		let fetchRequest = createClientSideRequest(init.history, path, abortController.signal, submission);
		fetchControllers.set(key, abortController);
		let actionResult = await callLoaderOrAction("action", fetchRequest, match, requestMatches, manifest, mapRouteProperties$1, basename);
		...
		let [matchesToLoad, revalidatingFetchers] = getMatchesToLoad(init.history, state, matches, submission, nextLocation, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, { [match.route.id]: actionResult.data }, void 0);
		...
		let redirect$1 = findRedirect(results);
		if (redirect$1) return startRedirectNavigation(state, redirect$1);
	...
	async function handleFetcherLoader(key, routeId, path, match, matches, submission) {
		...
		fetchControllers.set(key, abortController);
		let result = await callLoaderOrAction("loader", fetchRequest, match, matches, manifest, mapRouteProperties$1, basename);
	...
	*/
	async function startRedirectNavigation(state$1, redirect$1, _temp) {
		let { submission, replace, isFetchActionRedirect } = _temp === void 0 ? {} : _temp;
		if (redirect$1.revalidate) isRevalidationRequired = true;
		let redirectLocation = createLocation(state$1.location, redirect$1.location, _extends$2({ _isRedirect: true }, isFetchActionRedirect ? { _isFetchActionRedirect: true } : {}));
		invariant(redirectLocation, "Expected a location on the redirect navigation");
		if (ABSOLUTE_URL_REGEX$1.test(redirect$1.location) && isBrowser$1) {
			let url = init.history.createURL(redirect$1.location);
		...
		let activeSubmission = submission || getSubmissionFromNavigation(state$1.navigation);
		...
		else if (isFetchActionRedirect) await startNavigation(redirectHistoryAction, redirectLocation, {
	...
	async function callLoadersAndMaybeResolveData(currentMatches, matches, matchesToLoad, fetchersToLoad, request) {
		let results = await Promise.all([...matchesToLoad.map((match) => callLoaderOrAction("loader", request, match, matches, manifest, mapRouteProperties$1, basename)), ...fetchersToLoad.map((f) => {
			if (f.matches && f.match && f.controller) return callLoaderOrAction("loader", createClientSideRequest(init.history, f.path, f.controller.signal), f.match, f.matches, manifest, mapRouteProperties$1, basename);
			else return {
				type: ResultType.error,
	...
	function getScrollKey(location, matches) {
		if (getScrollRestorationKey) return getScrollRestorationKey(location, matches.map((m) => createUseMatchesMatch(m, state.loaderData))) || location.key;
		return location.key;
	}
...
function getMatchesToLoad(history, state, matches, submission, location, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionData, pendingError) {
	...
	fetchLoadMatches.forEach((f, key) => {
		...
		if (cancelledFetcherLoads.includes(key) || isPerformingInitialLoad || shouldRevalidateLoader(fetcherMatch, _extends$2({
			currentUrl,
...
async function callLoaderOrAction(type, request, match, matches, manifest, mapRouteProperties$1, basename, opts) {
	if (opts === void 0) opts = {};
	let resultType;
	let result;
	let onReject;
	...
	try {
		...
		if (match.route.lazy) if (handler) result = (await Promise.all([runHandler(handler), loadLazyRouteModule(match.route, mapRouteProperties$1, manifest)]))[0];
		else {
	...
	if (isResponse(result)) {
		...
		if (redirectStatusCodes.has(status)) {
			...
			if (!ABSOLUTE_URL_REGEX$1.test(location)) location = normalizeTo(new URL(request.url), matches.slice(0, matches.indexOf(match) + 1), basename, true, location);
...
function getShortCircuitMatches(routes) {
	let route = routes.find((r) => r.index || !r.path || r.path === "/") || { id: "__shim-error-route__" };
	return {
		matches: [{
			params: {},
			pathname: "",
...
var RenderErrorBoundary = class extends import_react.Component {
	...
	render() {
		return this.state.error ? /* @__PURE__ */ import_react.createElement(RouteContext.Provider, { value: this.props.routeContext }, /* @__PURE__ */ import_react.createElement(RouteErrorContext.Provider, {
...
function RouterProvider(_ref) {
	...
	return /* @__PURE__ */ import_react.createElement(import_react.Fragment, null, /* @__PURE__ */ import_react.createElement(DataRouterContext.Provider, { value: dataRouterContext }, /* @__PURE__ */ import_react.createElement(DataRouterStateContext.Provider, { value: state }, /* @__PURE__ */ import_react.createElement(Router, {
		...
		navigator
	}, state.initialized ? /* @__PURE__ */ import_react.createElement(DataRoutes, {
...
function Navigate(_ref4) {
	...
	warning(!import_react.useContext(NavigationContext).static, "<Navigate> must not be used on the initial render in a <StaticRouter>. This is a no-op, but you should modify your code so the <Navigate> is only ever rendered in response to some user interaction or state change.");
	let { matches } = import_react.useContext(RouteContext);
	let { pathname: locationPathname } = useLocation();
...
function deserializeErrors(errors) {
	...
	let serialized = {};
	for (let [key, val] of entries) if (val && val.__type === "RouteErrorResponse") serialized[key] = new ErrorResponse(val.status, val.statusText, val.data, val.internal === true);
...
function BrowserRouter(_ref) {
	let { basename, children, future, window: window$1 } = _ref;
	let historyRef = import_react.useRef();
...
function HashRouter(_ref2) {
	let { basename, children, future, window: window$1 } = _ref2;
	let historyRef = import_react.useRef();
...
function useSubmitFetcher(fetcherKey, fetcherRouteId) {
	...
	return import_react.useCallback(function(target, options) {
		...
		validateClientSideSubmission();
		let { action, method, encType, formData, body } = getFormSubmissionInfo(target, basename);
		!(fetcherRouteId != null) && invariant(false, "No routeId available for useFetcher()");
...
function useFetcher() {
	...
	let [Form$1] = import_react.useState(() => {
